...
def minimax(tablero, depth, is_maximizing, alpha, beta):
    ...
    if is_maximizing:
        ...
        for i in range(len(tablero)):
            for j in range(len(tablero[i])):
                if tablero[i][j] == ' ':
                    tablero[i][j] = 'O'
                    score = minimax(tablero, depth + 1, False, alpha, beta)
                    tablero[i][j] = ' '
        ...
        for i in range(len(tablero)):
            for j in range(len(tablero[i])):
                if tablero[i][j] == ' ':
                    tablero[i][j] = 'X'
                    score = minimax(tablero, depth + 1, True, alpha, beta)
                    tablero[i][j] = ' '
...
def mejor_movimiento(tablero):
    ...
    for i in range(len(tablero)):
        for j in range(len(tablero[i])):
            if tablero[i][j] == ' ':
                tablero[i][j] = 'O'
                score = minimax(tablero, 0, False, -1000, 1000)
                tablero[i][j] = ' '
                if score > best_score:
                    best_score = score
                    best_move = (i, j)
